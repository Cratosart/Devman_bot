...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(chat_id=update.effective_chat.id, text="Привет! Отправь /confirm для подтверждения получения уведомлений.")
...
async def confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if username in user_chat_ids:
        ...
        await context.bot.send_message(chat_id=chat_id, text="Твой chat ID подтверждён! Теперь ты будешь получать уведомления о проверках.")
    else:
        await context.bot.send_message(chat_id=update.effective_chat.id, text="Сначала используй команду /start.")
